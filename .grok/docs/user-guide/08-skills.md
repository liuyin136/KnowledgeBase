...
In the human-readable output, the Skills section lists each skill's name and its source -- `project`, `user`, or `plugin: <name>`. Grok tags any skill from a disabled vendor surface with `[disabled]`.
