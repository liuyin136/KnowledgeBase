...
> **Note:** `--tools`, `--disallowed-tools`, `--max-turns`, `--effort`, and `--agents` are headless-only flags. If used in the interactive TUI, a warning is printed and the flag is ignored. `--permission-mode`, `--allow`, and `--deny` work in both modes. For more flags (agents, verification, worktrees), see [Additional Headless Flags](#additional-headless-flags).
...
A single JSON object emitted after the response completes. Contains the full response text, stop reason, session ID, and request ID (plus a `thought` field when reasoning is present):
...
On failure, Grok instead emits an error object — check `type` before reading `text`:
