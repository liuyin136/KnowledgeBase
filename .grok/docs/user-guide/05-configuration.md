...
group_tool_verbs = false               # fold runs of read/search/list tool calls — and finished
                                       # thoughts among them — into one row (default: false)
...
timeout_secs = 360                     # seconds to wait when enabled (default: 360 / 6 min)
...
Each cell can also be toggled via environment variable (highest precedence) or remote feature flag (lowest). See the environment-variables reference for the env var names. Resolution order: env var > config.toml > feature flag > default (on).
...
|------|--------------------|
| `.grok/config.toml` | MCP servers, plugins, and permission rules (other sections load only from `~/.grok/config.toml`) |
