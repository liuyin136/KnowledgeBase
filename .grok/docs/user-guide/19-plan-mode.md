...
Grok is instructed to plan rather than edit: plan-file edits (`plan.md` in the session directory) are auto-approved, and other writes still require approval. Separating planning from implementation lets you review and correct the approach before any code is written.
...
| `Active`       | Plan mode is active. Plan-file edits are auto-approved; other writes still require approval. |
...
During active plan mode, edits to the plan file are auto-approved without prompting. This lets the agent iterate on the plan freely, while writes to other files still require your approval.
