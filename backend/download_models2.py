"""Build-time model downloader (no-op when DOWNLOAD_BGE=0)."""
...
DOWNLOAD_BGE = os.environ.get("DOWNLOAD_BGE", "0") == "1"
...
def main() -> None:
    ...
    if not DOWNLOAD_BGE:
        print(f"DOWNLOAD_BGE=0 — skipping model download; ensured {MODEL_PATH}")
        return
    try:
        ...
        sys.exit(1)
    print(f"DOWNLOAD_BGE=1 — downloaded model to {dest}")
